    ...
    <section className="py-12 md:py-24 bg-white">
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8">
        ...
        >
          ...
          <Button
            variant="outline"
            className="hidden md:flex rounded-full border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white transition-all font-medium px-6"
          >
                ...
                <div className="relative aspect-[3/4] md:aspect-[4/5] w-full overflow-hidden rounded-[1rem] bg-gray-200 isolate">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 z-10 flex flex-col justify-end">
                    ...
                    <div className="transform transition-transform duration-500 group-hover:-translate-y-2">
                      <h3 className="text-2xl md:text-3xl font-bold text-white mb-2 leading-none tracking-tight">{item.title}</h3>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {item.features.slice(0, 2).map((feature, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] font-bold text-white/90 uppercase tracking-wider bg-white/20 backdrop-blur-sm px-2 py-0.5 rounded border border-white/20"
                        >
                          {feature}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center justify-between gap-4 pt-4 border-t border-white/20">
                      <div>
                        <div className="flex items-baseline gap-1">
                          <span className="text-2xl font-black text-emerald-400 tracking-tight">{item.price}</span>
                          <span className="text-xs text-white/60 font-medium uppercase tracking-widest">{item.unit}</span>
                        </div>
                        {/* Animated Highlight */}
                      ...
                      <Button
                        size="icon"
                        className="h-12 w-12 rounded-full bg-white text-black hover:bg-emerald-500 hover:text-white transition-all duration-300 shadow-xl border-none"
                      >
