...
export function MobileBottomBar({
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 px-4 py-3 bg-white border-t border-gray-100 md:hidden z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] transition-transform duration-300 ${showMobileBar ? 'translate-y-0' : 'translate-y-full'}`}
    >
          ...
          {currentPrice > 0 ? (
            <>
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-black text-emerald-600 leading-none">₹{currentPrice.toLocaleString()}</span>
              </div>
            </>
          ) : (
